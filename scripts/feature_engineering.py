...
def create_sequences(
    df: pd.DataFrame, feature_cols: list, seq_length: int = 5
) -> tuple:
    """
...
def engineer_features(data_path: str, seq_length: int = 5) -> None:
    ...
    # If the dataset has only 1 observation per satellite, augment temporally.
    max_obs = df["NORAD_CAT_ID"].value_counts().max()
    if max_obs <= seq_length:
        logging.warning(
            f"Max observations per satellite ({max_obs}) <= seq_length ({seq_length})."
        )
        ...
        df = (
            ...
            .sort_values(by=["NORAD_CAT_ID", "EPOCH"])
    ...
    if len(X) > 0:
        ...
        logging.warning(
            "No sequences generated! Try decreasing seq_length or check data volume."
        )
