...
------------------------
To calculate distances between satellites, we must convert Orbital Elements to Cartesian (X, Y, Z):
...
3. Rotate from the orbital plane to the Geocentric Equatorial frame (ECI):
   ...
   X = x_orb * (cos(Ω)cos(ω) - sin(Ω)sin(ω)cos(i)) - y_orb * (cos(Ω)sin(ω) + sin(Ω)cos(ω)cos(i))
   Y = x_orb * (sin(Ω)cos(ω) + cos(Ω)sin(ω)cos(i)) - y_orb * (sin(Ω)sin(ω) - cos(Ω)cos(ω)cos(i))
   Z = x_orb * (sin(ω)sin(i)) + y_orb * (cos(ω)sin(i))

4. KD-Tree (K-Dimensional Tree):
   Instead of an O(N^2) brute-force distance check between all satellites,
   a KD-Tree recursively partitions 3D space. Finding neighbors within a distance D 
   becomes an O(N log N) spatial query, which is crucial for real-time collision detection.
"""
...
def keplerian_to_cartesian(df: pd.DataFrame) -> np.ndarray:
    ...
    X = x_orb * (np.cos(omega_node)*np.cos(omega_peri) - np.sin(omega_node)*np.sin(omega_peri)*np.cos(i)) \
        - y_orb * (np.cos(omega_node)*np.sin(omega_peri) + np.sin(omega_node)*np.cos(omega_peri)*np.cos(i))
        
    Y = x_orb * (np.sin(omega_node)*np.cos(omega_peri) + np.cos(omega_node)*np.sin(omega_peri)*np.cos(i)) \
        - y_orb * (np.sin(omega_node)*np.sin(omega_peri) - np.cos(omega_node)*np.cos(omega_peri)*np.cos(i))
        
    Z = x_orb * (np.sin(omega_peri)*np.sin(i)) + y_orb * (np.cos(omega_peri)*np.sin(i))
...
if __name__ == "__main__":
    ...
    # Run detection
    # Using a 200km threshold for demonstration so we actually find some pairs in a sparse 1000 object dataset
    ...
    if results:
        ...
        for r in results[:5]:
            print(f"[{r['distance_km']:.2f} km] {r['object_1_name']} (ID:{r['object_1_id']}) <--> {r['object_2_name']} (ID:{r['object_2_id']})")
    else:
